...
void workersWidget::on_refreshButton_clicked() {
    ...
    if (ui.sortBox->currentIndex() == 0 && ui.technicianCheckBox->checkState() == Qt::Checked) {  // Technicians only, Surname A - Z
        queryResult = workersQuery->executeQuery("SELECT id_pracownika, imie, nazwisko, nr_tel, stanowisko FROM pracownicy WHERE stanowisko='Serwisant' && id_pracownika NOT LIKE 1 ORDER BY nazwisko ASC");
    }
    else if (ui.sortBox->currentIndex() == 1 && ui.technicianCheckBox->checkState() == Qt::Unchecked) { //Surname Z - A
        queryResult = workersQuery->executeQuery("SELECT id_pracownika, imie, nazwisko, nr_tel, stanowisko FROM pracownicy WHERE id_pracownika NOT LIKE 1 ORDER BY nazwisko DESC");
    }
    else if (ui.sortBox->currentIndex() == 1 && ui.technicianCheckBox->checkState() == Qt::Checked) { // Technicians only, Surname Z - A
        queryResult = workersQuery->executeQuery("SELECT id_pracownika, imie, nazwisko, nr_tel, stanowisko FROM pracownicy WHERE stanowisko='Serwisant' && id_pracownika NOT LIKE 1 ORDER BY nazwisko DESC");
    }
    else {  // Surname A - Z
        queryResult = workersQuery->executeQuery("SELECT id_pracownika, imie, nazwisko, nr_tel, stanowisko FROM pracownicy WHERE id_pracownika NOT LIKE 1 ORDER BY nazwisko ASC");
    }
...
void workersWidget::on_addButton_clicked() {
    ...
    if (user.getPosition() != "Kierownik") {
        ...
        positionsResult = getPositions->executeQuery("SELECT stanowisko FROM pracownicy WHERE id_pracownika NOT LIKE 1 GROUP BY stanowisko;");  // find all worker positions currently existing in the database
            ...
            if (!addWorker.nameEdit->text().isEmpty() && !addWorker.surnameEdit->text().isEmpty() && !addWorker.phoneEdit->text().isEmpty() && !addWorker.passwordEdit->text().isEmpty() && !addWorker.loginEdit->text().isEmpty()) {
                if (addWorker.name2CheckBox->checkState() == Qt::Unchecked) {
                    if (addWorker.name2Edit->text().isEmpty()) {
                        ...
                        addWorkerQuery = con->prepareStatement("INSERT INTO pracownicy(imie, imie2, nazwisko, nr_tel, stanowisko, login, haslo) VALUES (?, ?, ?, ?, ?, ?, ?)");
                    ...
                    addWorkerQuery = con->prepareStatement("INSERT INTO pracownicy(imie, nazwisko, nr_tel, stanowisko, login, haslo) VALUES (?, ?, ?, ?, ?, ?)");
...
void workersWidget::on_deleteButton_clicked() {
    ...
    if (user.getPosition() != "Kierownik") {
        ...
        if (confirmation.exec()) {
            sql::Driver* driver;
            sql::Connection* con;
            sql::PreparedStatement* replaceWorker;
            ...
            // replace worker data with a reserved "empty" customer to prevent database errors
            replaceWorker = con->prepareStatement("UPDATE zlecenia SET id_pracownika=1 WHERE id_pracownika=?");
            replaceWorker->setInt(1, ui.workersList->currentItem()->data(Qt::UserRole).toInt());

            replaceWorker->execute();

            deleteWorker = con->prepareStatement("DELETE FROM pracownicy WHERE id_pracownika=? && id_pracownika NOT LIKE 1");
